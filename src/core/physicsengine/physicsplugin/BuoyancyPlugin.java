...
{
    private final static long serialVersionUID = -2381012703981263896L;


    @Override
    public void exertOn (GameElement s1, GameElement s2)
    {
        if (s1.getPhysicsAttribute().isPenetrable() ||
            s2.getPhysicsAttribute().isPenetrable())
        {
    ...
    {
        double fb =
            s1.getPhysicsAttribute().getMass() /
                    s1.getPhysicsAttribute().getDensity() *
                    s2.getPhysicsAttribute().getDensity() *
                    s1.getPhysicsAttribute().getGravitationalAcceleration();
