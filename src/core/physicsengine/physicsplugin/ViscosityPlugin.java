...
{
    private final static long serialVersionUID = 9057896007318235327L;
    ...
    {
        double fc = s2.getPhysicsAttribute().getDragCoefficient(), vx =
            s1.getVelocity().getX(), vy = s1.getVelocity().getY();
        double fx = fc * vx * vx;
        if (vx > 0) fx *= (-1);
        double fy = fc * vy * vy;
        if (vy > 0) fy *= (-1);
